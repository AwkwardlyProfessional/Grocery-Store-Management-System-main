...
def productList():
    ...
    productsList = []
    for (prod_id , name , prod_uom , prod_price) in mycursor.fetchall():
        productsList.append(
            {
                "prod_id" : prod_id, 
                "prod_name" : name, 
...
def addProduct(data):
    ...
    query = ("INSERT INTO proj_grocery_store.products ( name , prod_uom , prod_price) "
            " VALUES ( %s , %s , %s) ")                                                       # (8 , 'Tomato' , ''kg'' , 30) , prod_id is auto_increment so don't need to pass

    data = (data["name"] , data["prod_uom"] , data["prod_price"])
